...
const ViewPort: React.FC<ViewPortProps> = ({
    ...
    const handleMouseDown = (e: React.MouseEvent) => {
        ...
        if (!isAdmin || isShiftPressed) {
            setDragMode('PAN'); // Không phải admin hoặc Giữ Shift -> Kéo màn hình
    ...
    return (
        <div
            ref={viewportRef}
            className={`absolute z-20 top-0 left-0 w-full h-full min-h-full bg-[#1b1b1b] overflow-hidden select-none ${cursorClass} ${className || ''}`}
